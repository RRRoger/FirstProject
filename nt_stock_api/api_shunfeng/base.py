...
#         print 'data_digest',data_digest
        ...
        print '\n'
        print final_post_body_xml
        print '\n'
        _logger.info(final_post_body_xml)
        
        connection.request('POST', url, body=final_post_body_xml, headers=header)
        ...
        if isinstance(jsonobj,dict):
            ...
            if 'responseFail' in jsonobj and 'remark' in jsonobj['responseFail']:
                raise osv.except_osv(u"错误", "Message:%s" % jsonobj['responseFail']['remark'])
            if 'Response' in jsonobj and 'Body' in jsonobj['Response']:
                return jsonobj['Response']['Body']
            if 'Response' in jsonobj and 'Error' in jsonobj['Response']:
                raise osv.except_osv(u"错误", "Message:%s" % jsonobj['Response']['Error'])
        return jsonobj
